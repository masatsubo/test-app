...
function ProductList() {
  ...
  const [productsCutting, setProductsCutting] = useState({});
  const location = useLocation();

  useEffect(() => {
    if (location.state?.selectedCutting && location.state?.productId) {
      setProductsCutting(prev => ({
        ...prev,
        [location.state.productId]: location.state.selectedCutting
      }));
      setSelectedCategory('鮮魚');
    }
  }, [location.state]);
      ...
      <div className="products-grid">
        {products[selectedCategory].map(product => (
          <ProductItem 
            ...
            selectedCutting={productsCutting[product.id]}
          />
        ))}
      </div>
    </div>
